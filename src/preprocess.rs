...
pub fn preprocess(
    ...
    } else if let Types::FunctionCall(ref func_name, ref func_args) = element {
    ...
    } else if let Types::ArrayParsed(ref y) = element {
    ...
    } else if let Types::ArraySuite(ref y) = element {
        // matches multiple arrays following one another => implies array indexing
        let arrays: &Vec<Types> = y;
        let target_array: Types = process_stack(&vec![arrays[0].clone()], &variables, &functions);
