    ...
    for x in input {
        ...
        match x {
            ...
            Expr::Op(left, right) => {
                ...
                for x in op {
                    if let Expr::Opcode(op) = x {
                        if final_stack.is_empty() {
                            ...
                            crate::print!("2.OLD IS {:?}", final_stack.last().unwrap());
                ...
                crate::print!("CONSTS {consts:?}");
                crate::print!("VARS {variables:?}");
                crate::print!("FINAL STACK {final_stack:?}");
