...
#[macro_export] macro_rules! assert_args_number {
    ...
    ($func_name:expr, $received_args_len:expr, $min_args_len:expr, $max_args_len:expr) => {
        if $received_args_len < $min_args_len || $received_args_len > $max_args_len {
            error(&format!("Function '{}' expected between {} and {} arguments but received {}", $func_name, $min_args_len, $max_args_len, $received_args_len), "Remove the excess arguments");
        }
    }
}